...
def display_suite_comparison(
    ...
    for test_name in ref_results.results:
        if len(ref_results.results[test_name]) < 1 or len(tst_results.results[test_name]) < 1:
...
def _run_bench_mode(conf, verbose=False):
    # type: (TestConfig, bool) -> None
...
def _run_test_mode(conf, verbose=False):
    # type: (TestConfig, bool) -> None
...
def _run_compare_mode(conf, verbose=False):
    # type: (TestConfig, bool) -> None
...
def main(args):
    ...
    if args.mode == "test":
        ...
        _run_test_mode(conf, verbose=args.verbose)
    elif args.mode == "bench":
        conf.check_fields(("exe_path", "bench_out", "test_dirs", "query_dirs"))
        _run_bench_mode(conf, verbose=args.verbose)
    elif args.mode == "compare":
        conf.check_fields(("ref_dirs", "ref_ver", "cmp_ver"))
        _run_compare_mode(conf, verbose=args.verbose)
    elif args.mode == "clean":
        conf.check_fields(("test_out",))
        _run_bench_mode(conf)
    else:
